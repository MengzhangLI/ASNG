...
from nnunet.training.data_augmentation.augmentations import AutoAugment
matplotlib.use("agg")
...
class nnUNetTrainer(NetworkTrainer):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 unpack_data=True, deterministic=True, fp16=False, lam=2, gpu="0", save_dir=None):
        """
        :param deterministic:
        ...
        """
        super(nnUNetTrainer, self).__init__(deterministic, fp16, lam, gpu, save_dir)
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        self.AutoAugment = AutoAugment(self.data_aug_params[
                                           'patch_size_for_spatialtransform'], self.data_aug_params)
    ...
    def initialize_network_optimizer_and_scheduler(self):
        ...
        if len(gpu_ids) > 1:
            self.network = torch.nn.DataParallel(self.network.cuda(), device_ids=gpu_ids)
        else:
    ...
    def predict_preprocessed_data_return_softmax(self, data, do_mirroring, num_repeats, use_train_mode, batch_size,
        ...
        if isinstance(self.network, torch.nn.DataParallel):
            ...
            model = self.network
        return model.predict_3D(data, do_mirroring, num_repeats, use_train_mode, batch_size, mirror_axes,
