...
from datetime import datetime
from nnunet.training.data_augmentation.model import Model
import copy
...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False, lam=2, gpu=None, save_dir=None):
        """
        ...
        #self.file_open = open(os.path.join(self.save_dir, "log.pkl"), "wb")
    ...
    def print_to_log_file(self, *args, also_print_to_console=True, add_timestamp=True):
        ...
        if self.log_file is None:
            ...
            with open(self.log_file, 'w') as f:
                f.write("Starting... \n")
        self.file_open = open(os.path.join(self.output_folder, "log.pkl"), "wb")
        successful = False
        max_attempts = 5
        ctr = 0
    ...
    def run_training(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            for b in range(self.num_batches_per_epoch):
                l,theta = self.run_iteration(self.tr_gen, self.val_gen, True)
                train_losses_epoch.append(l)
                thetas.append(copy.deepcopy(theta))
            training_log.append(thetas)
            ...
            with torch.no_grad():
                ...
                for b in range(self.num_val_batches_per_epoch):
                    l = self.run_valid_iteration(self.val_gen, False, True)
                    val_losses.append(l)
                ...
                if self.also_val_in_tr_mode:
                    ...
                    val_losses = []
                    for b in range(self.num_val_batches_per_epoch):
                        l = self.run_valid_iteration(self.val_gen, False)
                        val_losses.append(l)
        ...
        pickle.dump(training_log, self.file_open)
    ...
    def run_iteration(self, data_generator, val_generator, do_backprop=True, run_online_evaluation=False):
        #data augmentation
        loss_sum = 0.
        data_dict = next(data_generator)
        val_dict = next(val_generator)
        #Parameters upgrade, Monte Carlo sampling.
        for i in range(self.lam):
            strategy = self.ASNGModel.forward(True)
            cur_data_dict = self.AutoAugment(data_dict, strategy)
            data = cur_data_dict['data']
            target = cur_data_dict['target']
        ...
        if do_backprop:
            if not self.fp16 or amp is None:
                ...
                with amp.scale_loss(loss_sum, self.optimizer) as scaled_loss:
                    scaled_loss.backward()
            self.optimizer.step()

        #Monte Carlo sampling , upgrading parameters of distribution...
        valid_data = val_dict['data']
        valid_target = val_dict['target']
        if not isinstance(valid_data, torch.Tensor):
            valid_data = torch.from_numpy(valid_data).float()
        if not isinstance(valid_target, torch.Tensor):
            valid_target = torch.from_numpy(valid_target).float()
        valid_data = valid_data.cuda(non_blocking=True)
        ...
        for i in range(self.lam):
            ...
            if do_backprop:
                ...
                self.optimizer.step()
            with torch.no_grad():
                valid_output = self.network(valid_data)
                valid_loss = self.loss(valid_output, valid_target)
                loss_all.append(valid_loss.cpu().item())
            self.network.load_state_dict(state_dict)
        loss_all = np.array(loss_all)
        Ms = np.array(Ms)
        self.ASNGModel.p_model_update(Ms, loss_all)

        with torch.no_grad():
            strategies = self.ASNGModel.forward(False)
            data_dict = self.AutoAugment(data_dict, strategies)
            data = data_dict['data']
            ...
            output = self.network(data)
            l = self.loss(output, target)
        return l.detach().cpu().numpy(), self.ASNGModel.p_model.theta

    def run_valid_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        data_dict = next(data_generator)
    ...
    def find_lr(self, num_iters=1000, init_value=1e-6, final_value=10., beta=0.98):
        ...
        for batch_num in range(1, num_iters + 1):
            raise ValueError("Wrong choose")
            # +1 because this one here is not designed to have negative loss...
            loss,_ = self.run_iteration(self.tr_gen, do_backprop=True, run_online_evaluation=False).data.item() + 1
